...
def main():
    ...
    <head>
        ...
        <style>
            ...
            body { font-family: -apple-system, "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", sans-serif; background: var(--bg); color: var(--text); margin: 0; line-height: 1.6; letter-spacing: 0.01em; }
            .container { max-width: 780px; margin: auto; padding: 10px; }
            
            header h1 { font-family: 'Noto Serif SC', serif; text-align: center; font-size: 26px; margin: 20px 0 15px; color: #1a1a1a; }
            
            /* 精简后的控制栏：去除背景和边框 */
            .control-bar { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; padding: 0 5px 10px 5px; border-bottom: 1px solid #f0f0f0; }
            .time-label { font-size: 11px; color: #888; font-weight: 400; }
            .date-picker { font-size: 11px; color: var(--primary); font-weight: bold; border: 1px solid #ddd; border-radius: 2px; padding: 2px 4px; background: transparent; cursor: pointer; }

            /* Tab 导航 */
            .tabs-nav { display: flex; margin-bottom: 20px; border-bottom: 1px solid #eee; }
            .tab-btn { padding: 10px 20px; cursor: pointer; border: none; background: none; font-size: 14px; font-weight: 600; color: #7f8c8d; transition: 0.2s; position: relative; }
            ...
            /* 今日头条 */
            .headline-section { background: #fff; padding: 16px; border: 1px solid var(--primary); margin-bottom: 25px; position: relative; border-radius: 2px; }
            .headline-label { background: #d93025; color: #fff; padding: 2px 8px; font-size: 10px; font-weight: bold; position: absolute; top: -10px; left: 12px; }
            .hl-item { border-bottom: 1px dashed #eee; padding: 12px 0; }
            .hl-item:last-child { border-bottom: none; }
            .hl-title { font-size: 18px; font-weight: 700; color: var(--primary); text-decoration: none; display: block; margin-bottom: 6px; font-family: 'Noto Serif SC', serif; }
            .hl-content { font-size: 14px; color: #34495e; line-height: 1.7; margin: 8px 0; }
            ...
            .co-title { 
                ...
                color: var(--primary); border-left: 5px solid var(--primary); 
                font-size: 18px; font-weight: 700; border-bottom: 1px solid #f0f0f0;
                font-family: 'Noto Serif SC', serif;
            }
            
            .news-item { padding: 12px 5px; border-bottom: 1px solid var(--border); cursor: pointer; }
            .news-item:hover { background: #fafafa; }
            
            .tag-group { margin-bottom: 6px; display: flex; gap: 8px; align-items: center; }
            .tag { font-size: 10px; padding: 1px 6px; font-weight: 600; background: #f1f3f4; color: #7f8c8d; border-radius: 2px; white-space: nowrap; }
            ...
            .title-row::after { content: '+'; font-size: 16px; color: #bdc3c7; }
            .news-item.open .title-row::after { content: '−'; color: var(--primary); }
            
            .content-box { display: none; padding: 10px 0; font-size: 13.5px; color: #444; line-height: 1.8; text-align: justify; }
            ...
            @media (max-width: 600px) {
                ...
                .control-bar { flex-direction: column; gap: 5px; align-items: center; }
            }
        </style>
    </head>
    <body>
    <div class="container">
        ...
        <div id="panel-daily" class="tab-content active">
            <div class="control-bar">
                ...
                <div>
                    <span style="font-size:11px; font-weight:bold; color:#888;">报告日期：</span>
            ...
            <div id="date-group-{{d}}" class="date-container" style="display: {{ 'block' if loop.first else 'none' }}">
                ...
                <div class="headline-section">
                    ...
                    <div class="hl-item">
                        ...
                        <div class="footer"><span>来源: {{hl['来源']}}</span><a href="{{hl['链接']}}" class="link-btn" target="_blank">阅读调查全文 →</a></div>
                    </div>
                    {% endfor %}
                </div>
                {% endif %}
                ...
                <div class="company-section">
                    ...
                    <div class="news-item" onclick="this.classList.toggle('open')">
                        ...
                        <div class="content-box">{{it['核心内容']}}<div class="footer"><span>来源: {{it['来源']}}</span><a href="{{it['链接']}}" class="link-btn" target="_blank" onclick="event.stopPropagation();">查看原文调查</a></div></div>
                    </div>
                    {% endfor %}
                </div>
                {% endfor %}
        ...
        <div id="panel-filter" class="tab-content">
            <div style="padding:15px 0; display:flex; gap:10px; margin-bottom:15px; position: sticky; top: 0; z-index: 101; background: #fff; border-bottom: 1px solid #eee;">
                ...
                <select id="f-co" style="flex:1; font-size:12px; padding:5px;"><option value="all">所有公司/模型</option>{% for c in all_companies %}<option value="{{c}}">{{c}}</option>{% endfor %}</select>
                <button onclick="doSearch()" style="background:var(--primary); color:white; border:none; padding:5px 20px; font-weight:bold; font-size:12px; border-radius:2px; cursor:pointer;">检索情报</button>
            </div>
            <div id="results"></div>
        </div>
    </div>
    ...
    <script>
        ...
        function doSearch() {
            ...
            resDiv.innerHTML = filtered.length ? '' : '<p style="text-align:center; padding:30px; font-size:12px; color:#999;">无结果</p>';
